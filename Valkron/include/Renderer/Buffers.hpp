...
template <> inline void VertexLayout::push<float>(unsigned int count) {
    m_elements.push_back({0x1406, count, 0});
    m_stride += count * VertexBufferElement::getSizeOfType(0x1406);
}

template <> inline void VertexLayout::push<unsigned int>(unsigned int count) {
    m_elements.push_back({0x1405, count, 0});
    m_stride += count * VertexBufferElement::getSizeOfType(0x1405);
}

template <> inline void VertexLayout::push<unsigned char>(unsigned int count) {
