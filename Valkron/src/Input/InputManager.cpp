...
void InputManager::pushLayer(Layer* layer) {
    ...
    LOG_INFO("Pushed layer " + std::to_string(layer->getLayerId()) +
             ". Total layers: " + std::to_string(m_layerStack.size()));
}
...
void InputManager::popLayer() {
    ...
    if (!m_layerStack.empty()) {
        ...
        LOG_INFO("Popped layer " + std::to_string(layer->getLayerId()) +
                 ". Remaining layers: " + std::to_string(m_layerStack.size()));
    } else {
...
glm::vec2 InputManager::getMousePosition() const {
    LOG_DEBUG("Getting mouse position: (" + std::to_string(m_mousePosition.x) + ", " +
              std::to_string(m_mousePosition.y) + ")");
...
void InputManager::glfwCharCallback(GLFWwindow* window, unsigned int codepoint) {
    auto* manager = static_cast<InputManager*>(glfwGetWindowUserPointer(window));
    if (!manager) {
        LOG_ERROR("GLFW char callback: InputManager pointer is null");
        return;
    }
...
void InputManager::glfwWindowSizeCallback(GLFWwindow* window, int width, int height) {
    auto* manager = static_cast<InputManager*>(glfwGetWindowUserPointer(window));
    if (!manager) {
        LOG_ERROR("GLFW window size callback: InputManager pointer is null");
        return;
    }

    LOG_DEBUG("GLFW window size callback: width=" + std::to_string(width) + ", height=" + std::to_string(height));
