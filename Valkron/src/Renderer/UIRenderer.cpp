...
struct UIRendererData {
    ...
    std::vector<UIDrawCommand> pendingCommands;
...
static bool uploadAtlasTexture(const std::vector<unsigned char>& atlasBytes, int width, int height) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, width, height, 0, GL_RED, GL_UNSIGNED_BYTE, atlasBytes.data());
...
static bool loadAtlasCache() {
    ...
    cacheFile.read(reinterpret_cast<char*>(cachedGlyphs.data()),
                   static_cast<std::streamsize>(cachedGlyphs.size() * sizeof(CachedGlyphInfo)));
    if (!cacheFile) {
        return false;
    }
...
static bool saveAtlasCache(const std::vector<unsigned char>& atlasBytes) {
    ...
    cacheFile.write(reinterpret_cast<const char*>(cachedGlyphs.data()),
                    static_cast<std::streamsize>(cachedGlyphs.size() * sizeof(CachedGlyphInfo)));
...
static void uploadBatch() {
    ...
    glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(s_data.pendingVertices.size() * sizeof(UIVertex)),
                 s_data.pendingVertices.data(), GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, s_data.ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizeiptr>(s_data.pendingIndices.size() * sizeof(std::uint32_t)),
                 s_data.pendingIndices.data(), GL_DYNAMIC_DRAW);
...
void UIRenderer::init() {
    ...
    if (!s_data.fontReady) {
        ...
        LOG_INFO("UIRenderer glyph sanity P: valid=" + std::to_string(glyphP.valid ? 1 : 0) +
                 ", advance=" + std::to_string(glyphP.advance) + ", size=" + std::to_string(glyphP.width) + "x" +
                 std::to_string(glyphP.height));
...
void UIRenderer::submitBatch(const std::vector<UIVertex>& vertices, const std::vector<std::uint32_t>& indices,
    ...
    s_data.pendingCommands = commands;
}
...
void UIRenderer::render(int viewportWidth, int viewportHeight) {
    ...
    if (s_data.pendingCommands.empty()) {
        glDisable(GL_SCISSOR_TEST);
        glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(s_data.pendingIndices.size()), GL_UNSIGNED_INT, nullptr);
    } else {
        for (const UIDrawCommand& command : s_data.pendingCommands) {
            if (command.indexCount == 0) {
                continue;
            }

            if (command.clipEnabled) {
                const int scissorX = static_cast<int>(std::round(command.clipRect.x));
                const int scissorW = static_cast<int>(std::round(command.clipRect.width));
                const int scissorH = static_cast<int>(std::round(command.clipRect.height));
                const int scissorY =
                    viewportHeight - static_cast<int>(std::round(command.clipRect.y + command.clipRect.height));
            ...
            glDrawElements(
                GL_TRIANGLES, static_cast<GLsizei>(command.indexCount), GL_UNSIGNED_INT,
                reinterpret_cast<void*>(static_cast<std::uintptr_t>(command.firstIndex) * sizeof(std::uint32_t)));
        }
...
void UIRenderer::appendTextGeometry(const std::string& text, const glm::vec2& position, float pixelHeight,
                                    const glm::vec4& color, std::vector<UIVertex>& outVertices,
