...
static bool loadAsciiPpmP3(const std::string& path, int& width, int& height, int& channels,
                           std::vector<unsigned char>& outPixels, bool flipVertical) {
...
bool Texture::loadTexture(const std::string& path, bool flipVertical) {
    ...
    if (!data) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, m_width, m_height, 0, GL_RGB, GL_UNSIGNED_BYTE, ppmPixels.data());
