...
void Renderer::endFrame() {
    ...
    s_data.frameBuffer->unbind();
    s_data.frameBuffer->blitToDefault(s_data.viewportWidth, s_data.viewportHeight, s_data.viewportWidth,
                                      s_data.viewportHeight, GL_COLOR_BUFFER_BIT);
    RenderCommand::setViewport(0, 0, s_data.viewportWidth, s_data.viewportHeight);
...
void Renderer::submitUIBatch(const std::vector<UIVertex>& vertices, const std::vector<std::uint32_t>& indices,
    ...
    UIRenderer::submitBatch(vertices, indices, commands);
}
...
void Renderer::onWindowResize(int width, int height) {
    if (width <= 0 || height <= 0) {
        LOG_WARN("Ignoring window resize with non-positive size: " + std::to_string(width) + "x" +
